void initBase(world &tanksWorld)
{
...
void initBird(world &tanksWorld, float offsetX, float offsetY)
{
    barrierStruct newWall;
...
void initSentBarrier(world &tanksWorld, float offsetX, float offsetY)
{
    barrierStruct newWall;
    newWall.box.setSize(sf::Vector2f{BIG_BARRIER_WIDTH, BIG_BARRIER_HEIGHT});
    newWall.box.setPosition(offsetX, offsetY);
...
void initMapBarriers(world &tanksWorld)
{
    ...
    for (float mapString = 0; mapString < MAP_HEIGHT; mapString++)
    {
        for (float mapColumn = 0; mapColumn < MAP_WIDTH; mapColumn++)
        {
            ...
            {
                case BASE:
                    initBird(tanksWorld, (mapColumn * BARRIER_WIDTH * 2) + FIELD_OFFSET_X, (mapString * BARRIER_HEIGHT * 2) + FIELD_OFFSET_Y);
                    break;
                case BRICK_WALL:
                ...
                case WATER_BLOCK:
                    ...
                    break;
                case SENT_BLOCK:
                    initSentBarrier(tanksWorld, (mapColumn * BARRIER_WIDTH * 2) + FIELD_OFFSET_X, (mapString * BARRIER_HEIGHT * 2) + FIELD_OFFSET_Y);
                    break;
                case ASPHALT_BLOCK:
...
void updateBarriers(world &tanksWorld)
{
    ...
    {
        if (barriers[i].type == BIRD && barriers[i].contactWithShell)
        {
