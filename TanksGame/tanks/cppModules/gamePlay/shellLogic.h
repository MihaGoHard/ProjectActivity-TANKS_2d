...
void updateShells(world &tanksWorld)
{
    for (int i = 0; i < tanksWorld.shells.size(); i++)
    {
        if (tanksWorld.shells[i].eraseShell)
        {
            ...
            {
                initExplosion(tanksWorld, shellPosition.x - EXPLOSION_OFFSET, shellPosition.y - EXPLOSION_OFFSET, BIG_EXPLOSION);
            }  
            else if (tanksWorld.shells[i].destroyedAimType == BARRIER)
            {
                initExplosion(tanksWorld, shellPosition.x - EXPLOSION_OFFSET, shellPosition.y - EXPLOSION_OFFSET, SIMPLE_EXPLOSION); 
            }  
...
void checkShellFieldBorderCollision(shell &currShell, bool &currShellDestroyed)
{
...
void checkShellTankCollision(shell &currShell, bool &currShellDestroyed, world &tanksWorld)
{
    for (int j = 0; j < tanksWorld.tanks.size(); j++)
    {
        ...
        if (hasCollision(currTank.box, currShell.box) && !currShellDestroyed)
        {
...
void checkShellsCollision(shell &currShell, bool &currShellDestroyed, world &tanksWorld)
{
    ...
    {
        if (currShell.id != tanksWorld.shells[n].id && hasCollision(currShell.box, tanksWorld.shells[n].box))
        {
            currShell.eraseShell = true;
...
void checkShellBarrierCollision(shell &currShell, bool &currShellDestroyed, world &tanksWorld)
{
    for (int i = 0; i < tanksWorld.barriers.size() && !currShellDestroyed; i++)
    {
        if (tanksWorld.barriers[i].type != FOREST && tanksWorld.barriers[i].type != WATER)
        {
            ...
            if (hasCollision(currShell.box, currBarrier))
            {
                if (tanksWorld.barriers[i].type == BASE_BIRD)
                {
                    currShell.destroyedAimType = TANK;
                }    
                else
                ...
                if (currShell.direction == RIGHT && hasCollision(currShell.box, downBarrier))
                {
                    tanksWorld.barriers[i + 3].contactWithShell = true;
                }    

                if (hasCollision(currShell.box, rightLeftBarrier))
                {
...
bool shellInShellsArr(int shellId, std::vector<shell> shells)
{
...
void checkShellCollision(world &tanksWorld)
{
    ...
    for (int i = 0; i < tanksWorld.shells.size(); i++)
    {
        ...
        {
            checkShellFieldBorderCollision(currShell, currShellDestroyed);
        }
